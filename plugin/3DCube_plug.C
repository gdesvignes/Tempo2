...
void WriteResults( long int counter, long double val, long double xx, 
                   long double yy, long double dd, int *map, 
                   int form, FILE *fout );
...
public:
  InputParams( int argc, char **argv, Param *XYZ, pulsar *psr ){
    ...
    // GD Fill in the struct
    for (int i=parDef; i<2; i++) {
        if( verb > 1 ){
	    cout << "pardef: " << i << endl;
	}
        XYZ[i].Init( "FIT", psr );
	XYZ[i].Values( 1.0, 1.0, 1.0 );
    }	
...
void CalcCube( Param *XYZ, long double *XX, long double *YY,
...
void FillArrays( long double *XX, long double *YY,               
...
void Resume( Param *XYZ, Param Dist, InputParams *IP, FILE *fout, pulsar *psr,
             long double *XX, long double *YY, long double *DD,
  ...
  *npsr = 1;
  ...
  for( ct = 0; ct < XYZ[0].Size(); ct++ ){
    ...
    for( int ct2 = 0; ct2 < XYZ[1].Size(); ct2++ ){
      Map[ct][ct2] = (long double *)malloc(  Dist.Size() *
                                             sizeof( long double ) );
  ...
  if( IP.res() == 1 ){
    ...
    Resume( XYZ, Dist, &IP, fout, psr, XX, YY, DD, Map, parFile[0], 
            timFile[0],parmap );
    if( IP.form() == 0 )
  ...
  // ==============================================================
  if( IP.plot() == 0 )
    CalcCube( XYZ, XX, YY, DD, Map, psr, npsr, fout, IP, parmap, Dist );
  ...
  OneDProject( XYZ, Dist, Map, XX, YY, DD );

  MakePlots( XYZ, Map, IP, Dist );

  free( XX ), free( YY ), free( DD );
  for( ct = 0; ct < XYZ[0].Size(); ct++ ){
    for( int ct2 = 0; ct2 < XYZ[1].Size(); ct2++ ){
      free( Map[ct][ct2] );
    }
...
void OneDProject( Param *XYZ, Param Dist, long double ***Map, long double *XX,
  ...
  for( xct = 0; xct < XYZ[0].Size(); xct++ ){
    Xmin[xct] = Map[xct][0][0];
    for( yct = 0; yct < XYZ[1].Size(); yct++ )
        for( dct = 0; dct < Dist.Size(); dct++ )
          if( Xmin[xct] == -1.0 || ( Map[xct][yct][dct] < Xmin[xct] &&
                                     Map[xct][yct][dct] != -1.0 ) )
            Xmin[xct] = Map[xct][yct][dct];
  }
  ...
  for( yct = 0; yct < XYZ[1].Size(); yct++ ){
    Ymin[yct] = Map[0][yct][0];
    for( xct = 0; xct < XYZ[0].Size(); xct++ )
        for( dct = 0; dct < Dist.Size(); dct++ )
          if( Ymin[yct] == -1.0 || ( Map[xct][yct][dct] < Ymin[yct] &&
                                     Map[xct][yct][dct] != -1.0 ) )
            Ymin[yct] = Map[xct][yct][dct];
  }
  ...
  for( dct = 0; dct < Dist.Size(); dct++ ){
    Dmin[dct] = Map[0][0][dct];
    for( xct = 0; xct < XYZ[0].Size(); xct++ )
      for( yct = 0; yct < XYZ[1].Size(); yct++ )
          if( Dmin[dct] == -1.0 || 
              ( Map[xct][yct][dct] < Dmin[dct] && 
                Map[xct][yct][dct] != -1.0 ) )
            Dmin[dct] = Map[xct][yct][dct];
  }
...
void PlotDist( Param *XYZ, long double ***Map, InputParams IP, Param Dist ){
  int ct, xct, yct, dct, mapct; // counters
  float *DMap;
  ...
  //cpgswin((float)Dist.I(), (float)Dist.A(), 0.0, max);
...
void PlotMapXY( Param *XYZ, long double ***Map, InputParams IP, Param Dist ){
  ...
  //cpgpage();
  //cpgsvp(0.1,0.9,0.1,0.9);
  //cpgswin((float)XYZ[0].I(), (float)XYZ[0].A(), (float)XYZ[1].I(), (float)XYZ[1].A());
  //cpgbox("BNST",0.0,0,"BCNST",0.0,0);
  cpglab( xstr, ystr, titstr );
  palett();
...
long double NormaliseMap( long double ***Map, Param *XYZ, Param Dist,
                          int *location ){
  long int xct, yct, dct; // generic counters
  ...
  for( xct = 0; xct < XYZ[0].Size(); xct++ )
    for( yct = 0; yct < XYZ[1].Size(); yct++ )
        for( dct = 0; dct < Dist.Size(); dct++ ){
          if( minimum == -1.0 || 
              ( Map[xct][yct][dct] < minimum 
                && Map[xct][yct][dct] != -1.0 ) ){
            minimum = Map[xct][yct][dct];
            location[0] = xct;
            location[1] = yct;
            location[3] = dct;
          }
          if( Map[xct][yct][dct] > maximum )
            maximum = Map[xct][yct][dct];
        }
...
void CalcCube( Param *XYZ, long double *XX, long double *YY,
  ...
  time_t start;
  double size = (double)( XYZ[0].Size() * XYZ[1].Size()
                          * Dist.Size() );
  time( &start );
    ...
    for( yct = 0; yct < XYZ[1].Size(); yct++ ){ // loop over y param.
	    ...
	    if (rv) {
	      ...
	      updateBatsAll( psr, *npsr );
            ...
            Map[xct][yct][dct] = psr[0].fitChisq;
            ...
            counter = dct + yct * Dist.Size()
              + xct * XYZ[1].Size() * Dist.Size();
            ...
            if( verb > 10 ){
              cout << "Cube XX: " << XX[xct] << " YY: " << YY[yct]
                   << " DD: " << DD[dct]
                   << " chisq: " << Map[xct][yct][dct] << endl;
              fflush( stdout );
            }
            WriteResults( counter, Map[xct][yct][dct], XX[xct], YY[yct], 
                          DD[dct], parmap, IP.form(), fout );
            calculated++;
...
void Resume( Param *XYZ, Param Dist, InputParams *IP, FILE *fout, pulsar *psr, 
  ...
  if( changed == 0 ){
    ...
    long int counter;
    long double Mapval, vals[2], dval;
    while( !feof( fout ) ){

      if( IP->form() == 0 ){
        // binary data
        rv = fread( &counter, sizeof( long int ), 1, fout );
        rv = fread( &Mapval, sizeof( long double ), 1, fout );
        rv = fread( &dval, sizeof( long double ), 1, fout );
        rv = fread( &vals[0], sizeof( long double ), 1, fout );
        rv = fread( &vals[1], sizeof( long double ), 1, fout );
      }else{
        // ascii data
        rv = fscanf( fout, "%ld %Lg %Lg %Lg %Lg\n", &counter, &Mapval, &dval,
                &vals[0], &vals[1]);
      }
...
void PutHeader( FILE *fout, Param *XYZ, InputParams IP, pulsar *psr,
  ...
  if( IP.form() == 0 ){
    ...
    sprintf( header, "%s %s %s %s %s %s\n", psr[0].name, parFile, timFile, 
             XYZ[0].name(), XYZ[1].name(), "binary" );
    fwrite( header, 100, 1, fout );
  }else{

    // ascii header
    fprintf( fout, "%s %s %s %s %s %s\n", psr[0].name, parFile, timFile,
             XYZ[0].name(), XYZ[1].name(), "ascii" );
  }
...
void Transform( InputParams *IP ){
  ...
  while( !feof( fin ) ){
    if( IP->form() == 1 ){
      ...
      rv = fscanf( fin, "%ld %Lg %Lg %Lg %Lg", &counter, &Mapval, &vals[0], &vals[1],
              &vals[2] );
      // now write:
      fwrite( &counter, sizeof( long int ), 1, fout );
      ...
      rv = fread( &counter, sizeof( long int ), 1, fin );
      rv = fread( &Mapval, sizeof( long double ), 1, fin );
      rv = fread( &vals[0], sizeof( long double ), 1, fin );
      rv = fread( &vals[1], sizeof( long double ), 1, fin );
      rv = fread( &vals[2], sizeof( long double ), 1, fin );
      // now write:
...
int ReadHeader( int form, char *fname, char *names[], char *psr, char *par,
  ...
  char format[100];
  rv = sscanf( read, "%s %s %s %s %s %s", psr, par, tim, names[0], names[1],
          format );
